...
def longest_substring_without_repeating_characters(s: str) -> int:
    """
    Length of the longest substring of `s` that contains no duplicate
    characters, using a single-pass O(n) dynamic programming approach.

    Time : O(n)          — one pass over the string.
    Space: O(min(n, Σ))  — the hash map holds at most one entry per distinct
                           character (bounded by the alphabet size Σ, or n
                           when the alphabet is unbounded).
    """
    ...
    for idx, char in enumerate(s):
        ...
        #
        # 2. idx - char_prev_index.get(char, -1):
