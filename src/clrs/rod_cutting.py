...
def cut_rod(prices: list[int]) -> int:
    ...
    for length in range(1, n + 1):
        ...
        #    (j = 1..length), each already solved optimally:
        #      dp[j] + dp[length - j]
        #    When j = length this term is dp[length] + dp[0] = 0
        #    (dp[length] still unset), so it does not contribute; the
        #    meaningful splits are j = 1..length-1. Because every further
        #    cut is already folded into dp[j] / dp[length-j], this covers
        #    all multi-piece partitions.
        #
        # 2. Sell the rod of this length uncut at prices[length].
        #
        # Taking the max of (1) and (2) is the rod-cutting recurrence.
        dp[length] = max(dp[j] + dp[length - j] for j in range(1, length + 1))
...
def cut_rod_price_seeded(prices: list[int]) -> int:
    ...
    (including j = length, which recovers the uncut price via
    `dp[length] + dp[0]` while `dp[length]` is still the seeded value).
    ...
    for length in range(1, len(prices)):
        ...
        # j runs from 1..length (j = 0 would duplicate j = length):
        #   - j = length → dp[length] + dp[0] = prices[length]
